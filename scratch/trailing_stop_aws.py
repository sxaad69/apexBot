...
class TrailingStopLayer:
        ...
        try:
            ...
            for trade_dict in open_trades:
                ...
                try:
                    ...
                    # 2. Trigger Exit Logic — differentiate trailing stop vs hard stop loss
                    trailing_stop_price = trade_dict.get('trailing_stop_price')
                    hard_stop_price = trade_dict.get('stop_loss')
                    ...
                    if trade_dict['side'] == 'buy':
                        if trailing_stop_price and current_price <= trailing_stop_price:
                            ...
                            self._trigger_close_position(trade_dict, current_price, reason='trailing_stop')
                        elif hard_stop_price and current_price <= hard_stop_price:
                            self.logger.system(f"🛑 Hard Stop Loss HIT for {symbol} (Long). Executing Market Close.")
                            self._trigger_close_position(trade_dict, current_price, reason='stop_loss')
                    elif trade_dict['side'] == 'sell':
                        if trailing_stop_price and current_price >= trailing_stop_price:
                            self.logger.system(f"🚨 Trailing Stop HIT for {symbol} (Short). Executing Market Close.")
                            self._trigger_close_position(trade_dict, current_price, reason='trailing_stop')
                        elif hard_stop_price and current_price >= hard_stop_price:
                            self.logger.system(f"🛑 Hard Stop Loss HIT for {symbol} (Short). Executing Market Close.")
                            self._trigger_close_position(trade_dict, current_price, reason='stop_loss')
    ...
    def _trigger_close_position(self, trade, current_price, reason='trailing_stop'):
